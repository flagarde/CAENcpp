...
namespace CAEN
{
  ...
  enum class Switch //CAEN_DGTZ_EnaDis_t;
  {
    ENABLE  = 1L,
    DISABLE = 0L,
  };
  
  enum class DPPFirmware
  {
    PHA,
    PSD,
    CI,
...
/**************************************************************************//**
* \fn       OpenDigitizer( const ConnectionType& LinkType, const int& LinkNum, const int& ConetNode, const std::uint32_t VMEBaseAddress);
* \brief    Open the Digitizer.
...
******************************************************************************/
void OpenDigitizer(const ConnectionType& LinkType,const int& LinkNum,const int ConetNode,const std::uint32_t& VMEBaseAddress=0);
...
/**************************************************************************//**
* \fn        IRQWait(std::uint32_t& timeout)
...
std::uint32_t ReadTemperature(const std::int32_t& ch);
    ...
    * \param     [OUT] DPPFirmware : Firmware type
    ******************************************************************************/
    DPPFirmware GetDPPFirmwareType();
  ...
  private:
    ...
    std::uint32_t m_FormFactor;
    std::uint32_t m_FamilyCode;
    std::string   m_ROC_FirmwareRel;
    std::string   m_AMC_FirmwareRel;
    std::uint32_t m_SerialNumber;
    std::array<std::string,4> m_MezzanineSerNum;       //used only for x743 boards
    std::uint32_t m_PCB_Revision;
    std::uint32_t m_ADC_NBits;
    std::uint32_t m_SAMCorrectionDataLoaded;        //used only for x743 boards
    int           m_CommHandle;
    int           m_VMEHandle;
